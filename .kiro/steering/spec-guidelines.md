...
- ❌ className の具体的な値
- ❌ 具体的な実装手順
- ❌ **データモデル（DynamoDB のキー設計・エンティティ構造）の書き写し**（下記参照）

### データモデルの扱い（SSOT厳守）

**データモデルの唯一の正は `.kiro/specs/database/design.md` である。**

機能 Spec の design.md に Data Models セクションを設ける場合、**参照のみ**にする。
キー設計やエンティティ構造をコピーしてはいけない。二重管理は必ず食い違う。

**✅ 良い例**:
```markdown
## Data Models

本機能が扱うエンティティの定義は SSOT を参照する。
**実装前に `.kiro/specs/database/design.md` を必ず読むこと。**

#[[file:.kiro/specs/database/design.md]]

本機能が使うエンティティ: `BODY`
本機能が使うアクセスパターン: A3（期間指定）
```

**注意**: `#[[file:]]` 記法は公式ドキュメント上 steering の機能としてのみ記載されており、
spec ファイルでの展開は確認できていない。展開されない場合はリテラル文字列として残るため、
**必ず「読むこと」という明示的な指示を併記する。**

**❌ 悪い例**: `interface BodyItem { pk: string; sk: string; weight: number ... }` を書き写す

**新しいエンティティが必要になった場合は、機能 Spec より先に
`.kiro/specs/database/design.md` を更新する。**

### インフラの扱い

インフラ専用の Spec 階層を作らない。詳細は `structure.md` の「インフラの扱い」を参照。
